...
import giga
import giga.tools as tools
import giga.release as release
from giga.tools import pycompat
...
# ad_paths is a deprecated alias, please use giga.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"giga.modules.module.ad_paths" is a deprecated proxy to '
        '"giga.addons.__path__".', DeprecationWarning, stacklevel=2)
    return giga.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "giga.addons".',
    ...
    def load_module(self, name):
        ...
        giga_name = re.sub(r'^openerp.addons.(\w+)$', r'giga.addons.\g<1>', name)

        giga_module = sys.modules.get(giga_name)
        if not giga_module:
            giga_module = importlib.import_module(giga_name)

        sys.modules[name] = giga_module

        return giga_module

class GigaHook(object):
    """ Makes giga package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to giga.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'giga\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `giga.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^giga.addons.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("giga.addons.base.maintenance.migrations", "giga.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``giga.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook giga.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in giga.addons.__path__:
        giga.addons.__path__.append(dd)

    # hook giga.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in giga.addons.__path__:
            giga.addons.__path__.append(ad)

    # hook giga.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in giga.addons.__path__ and os.path.isdir(base_path):
        giga.addons.__path__.append(base_path)

    # hook giga.upgrade on upgrade-path
    from giga import upgrade
    ...
    # create decrecated module alias from giga.addons.base.maintenance.migrations to giga.upgrade
    spec = importlib.machinery.ModuleSpec("giga.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["giga.addons.base.maintenance"] = maintenance_pkg
    sys.modules["giga.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to giga and "crm"-like to giga.addons
    if not getattr(initialize_sys_path, 'called', False): # only initialize once
        sys.meta_path.insert(0, UpgradeHook())
        sys.meta_path.insert(0, GigaHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in giga.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = giga.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in giga.addons.__path__:
...
def load_openerp_module(module_name):
    """ Load an GigaERP module, if not already loaded.
    ...
    try:
        __import__('giga.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['giga.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in giga.addons.__path__:
