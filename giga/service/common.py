...
RPC_VERSION_1 = {
        ...
        'server_serie': giga.release.serie,
        'protocol_version': 1,
}
...
def exp_about(extended=False):
    """Return information about the GigaERP Server.

    @param extended: if True then return version info
    @return string if extended is False else tuple
    """

    info = _('See http://gigasource.de')

    if extended:
        return info, giga.release.version
    return info

def exp_set_loglevel(loglevel, logger=None):
    # TODO Previously, the level was set on the now deprecated
    # `giga.netsvc.Logger` class.
    return True
