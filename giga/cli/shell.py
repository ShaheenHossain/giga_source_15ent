...
class Shell(Command):
    """Start giga in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        giga.cli.server.report_configuration()
        giga.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                ...
                ctx = giga.api.Environment(cr, uid, {})['res.users'].context_get()
                env = giga.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
