...
"""
GigaERP - Server
GigaERP is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - GigaERP SA
"""
...
__author__ = giga.release.author
__version__ = giga.release.version

# Also use the `giga` logger for the main script.
_logger = logging.getLogger('giga')
...
def report_configuration():
    ...
    """
    config = giga.tools.config
    _logger.info("Giga version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', giga.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = giga.tools.config
    if not giga.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = giga.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            giga.tools.trans_export(config["language"],
...
def main(args):
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                giga.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except giga.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        giga.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = giga.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the giga server (default command)"""
    def run(self, args):
        main(args)
