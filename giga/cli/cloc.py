...
from giga.tools import cloc, config
from . import Command

class Cloc(Command):
    """\
    Giga cloc is a tool to count the number of relevant lines written in
    ...
    It has two modes of operation, either by providing a path:
        ...
        giga-bin cloc -p module_path

    Or by providing the name of a database:

        giga-bin cloc --addons-path=dirs -d database
