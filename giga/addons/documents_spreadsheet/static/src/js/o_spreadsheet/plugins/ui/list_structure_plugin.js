...
export default class ListStructurePlugin extends spreadsheet.UIPlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            case "START":
                this._refreshGigaLists();
                break;
            case "BUILD_EAGLE_LIST":
            ...
            case "SELECT_EAGLE_LIST":
                this._selectList(cmd.listId);
                break;
            case "REFRESH_EAGLE_LIST":
                this._refreshGigaList(cmd.listId);
                break;
    ...
    _handleBuildList(sheetId, anchor, linesNumber, list, types) {
        this.dispatch("ADD_EAGLE_LIST", { list });
    ...
    _buildList(sheetId, anchor, listId, linesNumber, types) {
        ...
        for (const field_name of columns) {
            this.dispatch("ADD_EAGLE_LIST_FORMULA", {
                col,
                row,
                sheetId,
                formula: "LIST.HEADER",
