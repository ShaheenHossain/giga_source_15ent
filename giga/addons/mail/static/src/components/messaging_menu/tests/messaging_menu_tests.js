...
QUnit.test('rendering with GigaBot has a request (default)', async function (assert) {
    assert.expect(4);
    ...
    assert.ok(
        ...
        "should display a notification counter next to the messaging menu for GigaBot request"
    );
    assert.strictEqual(
        document.querySelector('.o_MessagingMenu_counter').textContent,
        "1",
    ...
    assert.strictEqual(
        ...
        'GigaBot has a request',
        "notification should display that GigaBot has a request"
    );
});

QUnit.test('rendering without GigaBot has a request (denied)', async function (assert) {
    assert.expect(2);
...
QUnit.test('rendering without GigaBot has a request (accepted)', async function (assert) {
    assert.expect(2);
