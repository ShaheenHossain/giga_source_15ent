...
from giga import _, api, exceptions, fields, models, tools, registry, SUPERUSER_ID, Command
from giga.exceptions import MissingError
from giga.osv import expression

from giga.tools.misc import clean_context, split_every
...
class MailThread(models.AbstractModel):
    ...
    def _message_route_process(self, message, message_dict, routes):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            if thread._name == 'mail.thread':  # message with parent_id not linked to record
                ...
                # parsing should find an author independently of user running mail gateway, and ensure it is not gigabot
    ...
    def message_post(self, *,
        ...
        if 'channel_ids' in kwargs:
            raise ValueError(_("Posting a message with channels as listeners is not supported since Giga 14.3+. Please update code accordingly."))
        ...
        if values['author_id'] and values['message_type'] != 'notification' and not self._context.get('mail_create_nosubscribe'):
            if self.env['res.partner'].browse(values['author_id']).active:  # we dont want to add gigabot/inactive as a follower
    ...
    def _message_auto_subscribe_followers(self, updated_values, default_subtype_ids):
        ...
        with track_visibility set. Since GigaERP v7 it is considered as being
