giga.define('l10n_de_pos_cert.PaymentScreen', function(require) {
    "use strict";
                ...
                this._handlePushOrderError = async (error) => {
                    if (error.source === 'fiskaly') {
                        const message = {
                            ...
                            'unknown': this.env._t('An unknown error has occurred ! Please contact Giga for more information.')
                        };
                    ...
                    if (this.currentOrder.isTransactionInactive()) {
                        await this.currentOrder.createTransaction().catch(async (error) => {
                            if (error.status === 0) {
                                ...
                                const message = {'unknown': this.env._t('An unknown error has occurred ! Please, contact Giga.')};
                                this.trigger('fiskaly-error', {error, message});
                            }
                        })
                    }
