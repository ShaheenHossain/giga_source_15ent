...
def _is_sent_by_amazon(mws_error):
    ...
    """
    if 700 <= mws_error.response.status_code <= 799:  # Custom Giga proxy error
        _logger.exception(
        ...
        if mws_error.response.status_code == 730:  # Forbidden
            raise exceptions.UserError(
                ...
                'https://www.gigasource.de/buy'
            )
        elif mws_error.response.status_code == 740:  # Bad Request
            raise exceptions.UserError(_("The Giga proxy received a malformed request."))
        elif mws_error.response.status_code == 750:  # Internal Error
            raise exceptions.UserError(_("The Giga proxy encountered an internal server error."))
        else:  # This is unexpected, we should probably have a custom message for this code
            raise exceptions.UserError(_("The Giga proxy encountered an unhandled error."))
