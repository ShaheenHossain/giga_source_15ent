...
from giga import api, exceptions, fields, models, _
from giga.osv import expression
from giga.service.model import PG_CONCURRENCY_ERRORS_TO_RETRY
...
class AmazonAccount(models.Model):
    ...
    auth_token = fields.Char(
        ...
        help="The MWS Authorization Token of the Amazon Seller Central account for Giga",
        required=True,
        groups="base.group_system")
    ...
    last_orders_sync = fields.Datetime(
        help="The last synchronization date for orders placed on this account. Orders whose status "
             "has not changed since this date will not be created nor updated in Giga.",
    ...
    def _build_get_api_connector_kwargs(self):
        ...
        in the request to the Giga proxy.
        """
        ...
        if not rate_limit_reached:
            try:
                ...
                if isinstance(error, psycopg2.OperationalError) and error.pgcode in PG_CONCURRENCY_ERRORS_TO_RETRY:
                    ...
                    # we let the default Giga's behavior happen, which is:
