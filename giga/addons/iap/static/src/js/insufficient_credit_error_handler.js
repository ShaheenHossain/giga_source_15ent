...
class InsufficientCreditDialog extends Dialog {
    ...
    async willStart() {
        ...
        if (errorData.trial && isEnterprise) {
            this.buttonMessage = _t("Start a Trial at Giga");
        } else {
...
InsufficientCreditDialog.bodyTemplate = "iap.redirect_to_giga_credit";
...
function insufficientCreditHandler(env, error, originalError) {
    ...
    if (data && data.name === "giga.addons.iap.tools.iap_tools.InsufficientCreditError") {
