...
giga.define('partner.autocomplete.Mixin', function (require) {
'use strict';
...
var PartnerAutocompleteMixin = {
    _dropPreviousGiga: new concurrency.DropPrevious(),
    ...
    _autocomplete: function (value) {
        ...
        return new Promise(function (resolve, reject) {
            var gigaPromise = self._getGigaSuggestions(value, isVAT).then(function (suggestions){
                gigaSuggestions = suggestions;
            });
            ...
            var concatResults = function () {
                // Add Clearbit result with Giga result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    var websites = gigaSuggestions.map(function (suggestion) {
                    ...
                    clearbitSuggestions.forEach(function (suggestion) {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            gigaSuggestions.push(suggestion);
                        }
                    });
                }

                gigaSuggestions = _.filter(gigaSuggestions, function (suggestion) {
                    return !suggestion.ignored;
                });
                _.each(gigaSuggestions, function(suggestion){
                delete suggestion.ignored;
                });
                return resolve(gigaSuggestions);
            };

            self._whenAll([gigaPromise, clearbitPromise]).then(concatResults, concatResults);
        });
    ...
    /**
     * Use Giga Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
    ...
    _getGigaSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousGiga.add(def);
    },
    /**
