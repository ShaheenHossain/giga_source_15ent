...
class SocialStreamFacebook(models.Model):
    ...
    def _format_facebook_message(self, message, tags):
        ...
        example:
            "Hello Giga :)" -> "Hello @[542132] Giga :)"
            "Hello Giga Social :)" -> "Hello @[542132] Giga-Social :)"
            "Hello Giga - Social :)" -> "Hello @[542132] Giga-Social :)"

        details:
            "Hello @[42] faketag Giga - Social :)" -> "Hello @ [42] faketag @[542132] Giga-Social :)""
            -> With associated tag: {name: "Giga - Social", offset: 20, length: 13}

            We take the message until the tag offset: "Hello @[42] faketag "
            and remove the fake tag in the process "Hello @ [42] faketag "
            Then we insert the forged tag "@[542132] Giga-Social"
