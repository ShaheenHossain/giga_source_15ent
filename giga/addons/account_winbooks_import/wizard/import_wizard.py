...
class WinbooksImportWizard(models.TransientModel):
    ...
    suspense_code = fields.Char(string="Suspense Account Code", help="This is the code of the account in which you want to put the counterpart of unbalanced moves. This might be an account from your Winbooks data, or an account that you created in Giga before the import.")
    ...
    def import_partner(self, file_dir, files, civility_data, category_data, account_data):
        ...
        :return: a dictionary whose keys are the Winbooks partner references and
            the values are the partner ids in Giga.
        """
    ...
    def import_account(self, file_dir, files, journal_data):
        ...
        :return: (account_data, account_central, account_deprecated_ids, account_tax)
            account_data is a dictionary whose keys are the Winbooks account
                references and the values are the account ids in Giga.
            account_central is a dictionary whose keys are the Winbooks central
                account references and the values are the account ids in Giga.
    ...
    def import_journal(self, file_dir, files):
        ...
        :return: a dictionary whose keys are the Winbooks journal references and
            the values the journal ids in Giga
        """
        ...
        for matching_number in reconcile_number_set:
            ...
            try:
                ...
                if len(lines.account_id) > 1:
                    _logger.warning('Winbooks matching number {} uses multiple accounts: {}. Lines with that number have not been reconciled in Giga.'.format(matching_number, ', '.join(lines.mapped('account_id.display_name'))))
    ...
    def import_analytic_account(self, file_dir, files):
        ...
        references and the values the analytic account ids in Giga.
        """
    ...
    def import_vat(self, file_dir, files, account_central):
        ...
        the values are the taxes ids in Giga.
        """
