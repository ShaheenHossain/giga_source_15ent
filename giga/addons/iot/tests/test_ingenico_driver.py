...
from giga.tools import file_open
...
class TestIncomingTransactionResponse(BaseCase):
    @patch.dict(
        "sys.modules", {
            ...
            "giga.addons.hw_drivers": MagicMock(),
            # Mock the modules IngenicoDriver imports so the imports don't fail
            "giga.addons.hw_drivers.driver": MagicMock(),
            "giga.addons.hw_drivers.event_manager": MagicMock(),
            "giga.addons.hw_drivers.iot_handlers.interfaces.SocketInterface": MagicMock(),
        }
    )
    def setUp(self):
        # pylint: disable=import-outside-toplevel
        from giga.addons.iot.iot_handlers.drivers.IngenicoDriver import IncomingIngenicoMessage
...
class TestOutgoingIngenicoMessage(BaseCase):
    ...
    def setUp(self):
        # pylint: disable=import-outside-toplevel
        from giga.addons.iot.iot_handlers.drivers.IngenicoDriver import OutgoingIngenicoMessage
