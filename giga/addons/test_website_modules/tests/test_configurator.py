...
import giga.tests
from giga.addons.iap.tools.iap_tools import iap_jsonrpc_mocked


@giga.tests.common.tagged('post_install', '-at_install')
class TestConfigurator(giga.tests.HttpCase):
    ...
    def setUp(self):
        ...
        iap_patch = patch('giga.addons.iap.tools.iap_tools.iap_jsonrpc', iap_jsonrpc_mocked_configurator)
        iap_patch.start()
        self.addCleanup(iap_patch.stop)

        patcher = patch('giga.addons.website.models.ir_module_module.IrModuleModule._theme_upgrade_upstream', wraps=self._theme_upgrade_upstream)
        patcher.start()
