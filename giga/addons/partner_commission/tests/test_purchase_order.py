...
from giga.addons.partner_commission.tests.setup import Line, Spec, TestCommissionsSetup
...
class TestPurchaseOrder(TestCommissionsSetup):
    def test_automatic_confirm(self):
        ...
        # Case: OK.
        with patch('giga.fields.Date.today', today):
            with patch('giga.addons.mail.models.mail_template.MailTemplate.send_mail', _patched_send_mail):
    ...
    def test_purchase_representative(self):
        ...
        with self.subTest("Each sales representative has its own PO."):
            sales_rep = self.env['res.users'].create({
                'name': '...',
                'login': 'sales_rep_1',
                'email': 'sales_rep_1@gigasource.de',
                'company_id': self.company.id,
        ...
        with self.subTest("Subscription's salesperson takes precedence over SO's salesperson."):
            sales_rep = self.env['res.users'].create({
                'name': '...',
                'login': 'sales_rep_2',
                'email': 'sales_rep_2@gigasource.de',
                'company_id': self.company.id,
                'groups_id': [(6, 0, [self.ref('sales_team.group_sale_salesman')])],
            })
