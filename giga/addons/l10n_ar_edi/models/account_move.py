...
from giga.tools.float_utils import float_repr, float_round
...
class AccountMove(models.Model):
    ...
    def _post(self, soft=True):
        ...
        for inv in sale_ar_edi_invoices:
            ...
            # in Giga for consistency, this way if an error ocurrs later in another invoice we will have the ones
            # correctly validated in AFIP in Giga (CAE, Result, xml response/request).
...
""",
    ...
    def _l10n_ar_check_afip_auth_verify_required(self):
        ...
        if still_missing:
            ...
            raise UserError(_('We can not post %s in Giga because the AFIP verification fail: %s\nPlease verify in AFIP manually'
