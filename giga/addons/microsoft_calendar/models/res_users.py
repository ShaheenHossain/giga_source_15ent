...
import logging
import requests
from giga.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
...
from giga.addons.microsoft_calendar.utils.microsoft_calendar import MicrosoftCalendarService, InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_microsoft_calendar(self, calendar_service: MicrosoftCalendarService):
        ...
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2giga(events, default_reminders=default_reminders) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # Giga -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_giga2microsoft(calendar_service)
        ...
        (events - synced_events)._sync_giga2microsoft(calendar_service)
