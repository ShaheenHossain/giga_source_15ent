...
from giga.tools import html2plaintext, is_html_empty, plaintext2html
...
class Meeting(models.Model):
    ...
    @api.model
    def _microsoft_to_giga_values(self, microsoft_event, default_reminders=(), default_values={}):
        ...
        commands_attendee, commands_partner = self._giga_attendee_commands_m(microsoft_event)
        ...
        alarm_commands = self._giga_reminders_commands_m(microsoft_event)
        if alarm_commands:
    ...
    @api.model
    def _microsoft_to_giga_recurrence_values(self, microsoft_event, default_reminders=(), values={}):
        timeZone_start = pytz.timezone(microsoft_event.start.get('timeZone'))
        timeZone_stop = pytz.timezone(microsoft_event.end.get('timeZone'))
        start = parse(microsoft_event.start.get('dateTime')).astimezone(timeZone_start).replace(tzinfo=None)
        if microsoft_event.isAllDay:
    ...
    @api.model
    def _giga_attendee_commands_m(self, microsoft_event):
        commands_attendee = []
        commands_partner = []
        ...
        if microsoft_event.exists(self.env):
            existing_attendees = self.env['calendar.attendee'].search([
                ('event_id', '=', microsoft_event.giga_id(self.env)),
                ('email', 'in', emails)])
        ...
        for giga_attendee in attendees_by_emails.values():
            # Remove old attendees
            if giga_attendee.email not in emails:
                commands_attendee += [(2, giga_attendee.id)]
                commands_partner += [(3, giga_attendee.partner_id.id)]
        return commands_attendee, commands_partner

    @api.model
    def _giga_reminders_commands_m(self, microsoft_event):
        reminders_commands = []
        if microsoft_event.isReminderOn:
            event_id = self.browse(microsoft_event.giga_id(self.env))
    ...
    def _microsoft_values(self, fields_to_sync, initial_values={}):
        ...
        values['singleValueExtendedProperties'] = [{
            'id': 'String {%s} Name giga_id' % microsoft_guid,
            'value': str(self.id),
        }, {
            'id': 'String {%s} Name owner_giga_id' % microsoft_guid,
    ...
    def _ensure_attendees_have_email(self):
        ...
        if invalid_event_ids:
            ...
            raise ValidationError(_("For a correct synchronization between Giga and Outlook Calendar, "
