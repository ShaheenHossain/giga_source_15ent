...
from giga import api, fields, models, registry, _
from giga.tools import ormcache_context
from giga.exceptions import UserError
from giga.osv import expression

from giga.addons.microsoft_calendar.utils.microsoft_event import MicrosoftEvent
from giga.addons.microsoft_calendar.utils.microsoft_calendar import MicrosoftCalendarService
from giga.addons.microsoft_account.models.microsoft_service import TIMEOUT
...
# This ensures changes are sent to Microsoft only if they really happened in the Giga database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Microsoft if the first creation crashed in Giga.
...
class MicrosoftSync(models.AbstractModel):
    ...
    def _sync_giga2microsoft(self, microsoft_service: MicrosoftCalendarService):
        if not self:
            return
    ...
    def _sync_recurrence_microsoft2giga(self, microsoft_events: MicrosoftEvent):
        ...
        for recurrent_master in recurrent_masters:
            new_calendar_recurrence = dict(self.env['calendar.recurrence']._microsoft_to_giga_values(recurrent_master, (), default_values), need_sync_m=False)
            to_create = recurrents.filter(lambda e: e.seriesMasterId == new_calendar_recurrence['microsoft_id'])
            recurrents -= to_create
            base_values = dict(self.env['calendar.event']._microsoft_to_giga_values(recurrent_master, (), default_values), need_sync_m=False)
            to_create_values = []
            ...
            for recurrent_event in to_create:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_giga_recurrence_values(recurrent_event, (), base_values)
                else:
                    value = self.env['calendar.event']._microsoft_to_giga_values(recurrent_event, (), default_values)
            ...
            new_recurrence_giga = self.env['calendar.recurrence'].create(new_calendar_recurrence)
            new_recurrence_giga.base_event_id = new_recurrence_giga.calendar_event_ids[0] if new_recurrence_giga.calendar_event_ids else False
            new_recurrence |= new_recurrence_giga
        ...
        for recurrent_master_id in set([x.seriesMasterId for x in recurrents]):
            ...
            for recurrent_event in to_update:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_giga_recurrence_values(recurrent_event, (), {'need_sync_m': False})
                else:
                    value = self.env['calendar.event']._microsoft_to_giga_values(recurrent_event, (), default_values)
    ...
    def _update_microsoft_recurrence(self, recurrence_event, events):
        vals = dict(self.base_event_id._microsoft_to_giga_values(recurrence_event, ()), need_sync_m=False)
        ...
        for recurrent_event in events_to_update:
            if recurrent_event.type == 'occurrence':
                value = self.env['calendar.event']._microsoft_to_giga_recurrence_values(recurrent_event, (), default_values)
                normal_events += [recurrent_event.giga_id(self.env)]
            else:
                value = self.env['calendar.event']._microsoft_to_giga_values(recurrent_event, (), default_values)
                self.env['calendar.event'].browse(recurrent_event.giga_id(self.env)).with_context(no_mail_to_attendees=True, mail_create_nolog=True).write(dict(value, need_sync_m=False))
    ...
    @api.model
    def _sync_microsoft2giga(self, microsoft_events: MicrosoftEvent, default_reminders=()):
        """Synchronize Microsoft recurrences in Giga. Creates new recurrences, updates
        existing ones.

        :return: synchronized giga
        """
        ...
        giga_values = [
            dict(self._microsoft_to_giga_values(e, default_reminders, default_values), need_sync_m=False)
            for e in (new - new_recurrent)
        ]
        new_giga = self.with_context(dont_notify=True)._create_from_microsoft(new, giga_values)

        synced_recurrent_records = self._sync_recurrence_microsoft2giga(new_recurrent)

        cancelled = existing.cancelled()
        cancelled_giga = self.browse(cancelled.giga_ids(self.env))
        cancelled_giga._cancel_microsoft()
        ...
        synced_records = new_giga + cancelled_giga + synced_recurrent_records.calendar_event_ids

        for mevent in (existing - cancelled).filter(lambda e: e.lastModifiedDateTime and not e.seriesMasterId):
            # Last updated wins.
            # This could be dangerous if microsoft server time and giga server time are different
            if mevent.is_recurrence():
                giga_record = self.env['calendar.recurrence'].browse(mevent.giga_id(self.env))
            else:
                giga_record = self.browse(mevent.giga_id(self.env))
            giga_record_updated = pytz.utc.localize(giga_record.write_date)
            updated = parse(mevent.lastModifiedDateTime or str(giga_record_updated))
            if updated >= giga_record_updated:
                vals = dict(giga_record._microsoft_to_giga_values(mevent, default_reminders), need_sync_m=False)
                giga_record._write_from_microsoft(mevent, vals)
                if giga_record._name == 'calendar.recurrence':
                    giga_record._update_microsoft_recurrence(mevent, microsoft_events)
                    synced_recurrent_records |= giga_record
                else:
                    synced_records |= giga_record
    ...
    def _get_microsoft_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Giga Source to Microsoft
    ...
    @api.model
    def _microsoft_to_giga_values(self, microsoft_event: MicrosoftEvent, default_reminders=()):
        """Implements this method to return a dict of Giga values corresponding
        to the Microsoft event given as parameter
        :return: dict of Giga formatted values
        """
