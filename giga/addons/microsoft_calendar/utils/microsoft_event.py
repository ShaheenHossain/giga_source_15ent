...
class MicrosoftEvent(abc.Set):
    ...
    Inspired by Giga recordset, one instance can be a single Microsoft event or a
    ...
    def giga_id(self, env):
        self.giga_ids(env)  # load ids
        return self._giga_id

    def _meta_giga_id(self, microsoft_guid):
        """Returns the Giga id stored in the Microsoft Event metadata.
        This id might not actually exists in the database.
        """
        if self.singleValueExtendedProperties:
            o_id = [prop['value'] for prop in self.singleValueExtendedProperties if prop['id'] == 'String {%s} Name giga_id' % microsoft_guid][0]
            return int(o_id)

    def giga_ids(self, env):
        ids = tuple(e._giga_id for e in self if e._giga_id)
        if len(ids) == len(self):
            return ids
        found = self._load_giga_ids_from_db(env)
        unsure = self - found
        if unsure:
            unsure._load_giga_ids_from_metadata(env)

        return tuple(e._giga_id for e in self)

    def _load_giga_ids_from_metadata(self, env):
        ...
        unsure_giga_ids = tuple(e._meta_giga_id(microsoft_guid) for e in self)
        giga_events = model_env.browse(_id for _id in unsure_giga_ids if _id)

        # Extended properties are copied when splitting a recurrence Microsoft side.
        # Hence, we may have two Microsoft recurrences linked to the same Giga id.
        # Therefore, we only consider Giga records without microsoft id when trying
        # to match events.
        o_ids = giga_events.exists().filtered(lambda e: not e.microsoft_id).ids
        for e in self:
            giga_id = e._meta_giga_id(microsoft_guid)
            if giga_id in o_ids:
                e._events[e.id]['_giga_id'] = giga_id

    def _load_giga_ids_from_db(self, env):
        model_env = self._get_model(env)
        giga_events = model_env.with_context(active_test=False)._from_microsoft_ids(self.ids).with_env(env)
        mapping = {e.microsoft_id: e.id for e in giga_events}
        existing_microsoft_ids = giga_events.mapped('microsoft_id')
        for e in self:
            giga_id = mapping.get(e.id)
            if giga_id:
                e._events[e.id]['_giga_id'] = giga_id
        return self.filter(lambda e: e.id in existing_microsoft_ids)

    def owner(self, env):
        # Owner/organizer could be desynchronised between Microsoft and Giga.
        # Let userA, userB be two new users (never synced to Microsoft before).
        # UserA creates an event in Giga (he is the owner) but userB syncs first.
        ...
        # (he is the orginizer in Microsoft). The "real" owner (in Giga) is stored as an
        # extended property. There is currently no support to "transfert" ownership when
        # userA syncs his calendar the first time.
        if self.singleValueExtendedProperties:
            microsoft_guid = env['ir.config_parameter'].sudo().get_param('microsoft_calendar.microsoft_guid', False)
            real_owner_id = [prop['value'] for prop in self.singleValueExtendedProperties if prop['id'] == 'String {%s} Name owner_giga_id' % microsoft_guid][0]
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Microsoft: 1 email = 1 user; but in Giga several users might have the same email
    ...
    def exists(self, env) -> 'MicrosoftEvent':
        ...
        recurrences.giga_ids(env)
        events.giga_ids(env)

        return self.filter(lambda e: e._giga_id)
