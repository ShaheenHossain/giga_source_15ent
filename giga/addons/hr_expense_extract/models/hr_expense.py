...
class HrExpense(models.Model):
    ...
    def action_submit_expenses(self, **kwargs):
        ...
        for expense in self.filtered(lambda x: x.extract_state == 'waiting_validation'):
            endpoint = self.env['ir.config_parameter'].sudo().get_param(
                'hr_expense_extract_endpoint', 'https://iap-extract.gigasource.de') + '/iap/expense_extract/validate'
    ...
    def _check_status(self):
        self.ensure_one()
        endpoint = self.env['ir.config_parameter'].sudo().get_param(
            'hr_expense_extract_endpoint', 'https://iap-extract.gigasource.de') + '/iap/expense_extract/get_result'
        params = {
                'version': CLIENT_OCR_VERSION,
    ...
    def retry_ocr(self):
        ...
        if attachments and attachments.exists() and self.extract_state in ['no_extract_requested', 'not_enough_credit', 'error_status', 'module_not_up_to_date']:
            ...
            endpoint = self.env['ir.config_parameter'].sudo().get_param(
                    'hr_expense_extract_endpoint', 'https://iap-extract.gigasource.de') + '/iap/expense_extract/parse'
...
<p>Snap pictures of your receipts and let Giga<br/> automatically create expenses for you.</p>
<p>
    <a href="https://apps.apple.com/be/app/giga/id1272543640" target="_blank">
        <img alt="Apple App Store" class="img img-fluid h-100 o_expense_apple_store" src="/hr_expense/static/img/app_store.png"/>
    </a>
    <a href="https://play.google.com/store/apps/details?id=com.giga.mobile" target="_blank">
