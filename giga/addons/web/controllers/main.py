...
import giga
import giga.modules.registry
from giga.api import call_kw
from giga.addons.base.models.qweb import QWeb
from giga.modules import get_resource_path, module
from giga.tools import html_escape, pycompat, ustr, apply_inheritance_specs, lazy_property, float_repr, osutil
from giga.tools.mimetypes import guess_mimetype
from giga.tools.translate import _
from giga.tools.misc import str2bool, xlsxwriter, file_open, file_path
from giga.tools.safe_eval import safe_eval, time
from giga import http
from giga.http import content_disposition, dispatch_rpc, request, serialize_exception as _serialize_exception
from giga.exceptions import AccessError, UserError, AccessDenied
from giga.models import check_method_name
from giga.service import db, security
...
def fix_view_modes(action):
    """ For historical reasons, Giga has weird dealings in relation to
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = giga.SUPERUSER_ID

        values = request.params.copy()
        try:
            values['databases'] = http.db_list()
        except giga.exceptions.AccessDenied:
        ...
        if request.httprequest.method == 'POST':
            ...
            except giga.exceptions.AccessDenied as e:
                request.uid = old_uid
                if e.args == giga.exceptions.AccessDenied().args:
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = giga.SUPERUSER_ID
...
class WebClient(http.Controller):
    ...
    def qweb(self, unique, mods=None, db=None, bundle=None):
        ...
        if not request.db and mods is None:
            mods = giga.conf.server_wide_modules or []
    ...
    def translations(self, unique, mods=None, lang=None):
        ...
        if mods:
            ...
            mods = list(request.env.registry._init_modules) + (giga.conf.server_wide_modules or [])
    ...
    def version_info(self):
        return giga.service.common.exp_version()
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = giga.tools.config.verify_admin_password('admin')
        d['list_db'] = giga.tools.config['list_db']
        d['langs'] = giga.service.db.exp_list_lang()
        d['countries'] = giga.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = giga.service.db.list_db_incompatible(d['databases'])
        except giga.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = giga.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            giga.service.db.check_super(master_pwd)
            ...
            dump_stream = giga.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def account(self):
        ...
        return 'https://accounts.gigasource.de/oauth2/auth?' + url_encode(params)
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = giga.modules.registry.Registry(dbname)
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: giga.tools.ustr(field[1].get('string', '').lower()))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from Giga Source's export data to whatever the
