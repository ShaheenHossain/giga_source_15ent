...
QUnit.module("ActionManager", (hooks) => {
    ...
    QUnit.test("flags field of ir.actions.act_window is used", async function (assert) {
        // more info about flags field : https://github.com/giga/gigasource.demit/c9b133813b250e89f1f61816b0eabfb9bee2009d
        assert.expect(7);
    ...
    QUnit.test("window action in target new fails (onchange)", async (assert) => {
        ...
        registry
            ...
            .add("giga.exceptions.ValidationError", WarningDialogWait);

        const mockRPC = (route, args) => {
            if (args.method === "onchange" && args.model === "partner") {
                const error = new RPCError();
                error.exceptionName = "giga.exceptions.ValidationError";
                error.code = 200;
