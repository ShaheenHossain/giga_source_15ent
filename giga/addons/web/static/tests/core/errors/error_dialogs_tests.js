...
QUnit.test("ErrorDialog with traceback", async (assert) => {
    ...
    assert.strictEqual(target.querySelector("header .modal-title").textContent, "Giga Error");
...
QUnit.test("WarningDialog", async (assert) => {
    ...
    class Parent extends Component {
        constructor() {
            super(...arguments);
            this.name = "giga.exceptions.UserError";
            this.message = "...";
...
QUnit.test("RedirectWarningDialog", async (assert) => {
    ...
    parent = await mount(Parent, { env, target });
    assert.containsOnce(target, ".o_dialog");
    assert.strictEqual(target.querySelector("header .modal-title").textContent, "Giga Warning");
    assert.strictEqual(target.querySelector("main").textContent, "Some strange unreadable message");
...
QUnit.test("SessionExpiredDialog", async (assert) => {
    ...
    assert.strictEqual(
        target.querySelector("header .modal-title").textContent,
        "Giga Session Expired"
    );
    assert.strictEqual(
        target.querySelector("main p").textContent,
        " Your Giga session expired. The current page is about to be refreshed. "
    );
