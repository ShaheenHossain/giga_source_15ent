...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Giga" });
    assert.strictEqual(env.services.title.current, "Giga");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Giga", chat: null });
    assert.strictEqual(env.services.title.current, "Giga");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Giga - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Giga", action: "Import" });
    assert.strictEqual(env.services.title.current, "Giga - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Giga", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Giga - Import"); // parts is a copy!
});
