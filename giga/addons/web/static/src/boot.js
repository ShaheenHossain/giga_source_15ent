...
(function () {
    ...
    var giga = window.giga;
    var debug = giga.debug;
    ...
    giga.testing = typeof QUnit === "object";
    giga.remainingJobs = jobs;
    giga.__DEBUG__ = {
    ...
    giga.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === "string" ? args.shift() : "__giga_job" + jobUID++;
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: giga.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name,
                };
                if (jobs[k].error) {
                ...
                var deps = giga.__DEBUG__.getDependencies(job.name);
            ...
            missing = giga.__DEBUG__.getMissingJobs();
            failed = giga.__DEBUG__.getFailedJobs();
    ...
    giga.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec)
                        .then(function (data) {
                            ...
                            resolve();
                            giga.processJobs(jobs, services);
                        })
