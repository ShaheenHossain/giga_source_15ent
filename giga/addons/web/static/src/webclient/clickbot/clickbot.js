...
(function (exports) {
    ...
    const BLACKLISTED_MENUS = [
        ...
        "payment_giga.menu_adyen_account",
        "payment_giga.root_adyen_menu",
    ];

    const { isEnterprise } = giga.info;
    ...
    function ensureSetup() {
        ...
        setupDone = true;
        const env = giga.__WOWL_DEBUG__.root.env;
        env.bus.on("ACTION_MANAGER:UI-UPDATED", null, () => {
            actionCount++;
        });

        const AbstractController = giga.__DEBUG__.services["web.AbstractController"];
        ...
        const { patch } = giga.__DEBUG__.services["@web/core/utils/patch"];
        const { WithSearch } = giga.__DEBUG__.services["@web/search/with_search/with_search"];
        ...
        // This test file is not respecting Giga module dependencies.
        // The following module might not be loaded (eg. if mail is not installed).
        const DiscussWidgetModule = giga.__DEBUG__.services["@mail/widgets/discuss/discuss"];
    ...
    async function _clickEverywhere(xmlId) {
        ...
        console.log(`Giga flavor: ${isEnterprise ? "Enterprise" : "Community"}`);
