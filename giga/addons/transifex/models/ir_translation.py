...
class IrTranslation(models.Model):
    ...
    transifex_url = fields.Char("Transifex URL", compute='_get_transifex_url', help="Propose a modification in the official version of Giga")

    def _get_transifex_url(self):
        """ Construct transifex URL based on the module on configuration """
        # e.g. 'https://www.transifex.com/giga/'
        ...
        for addon_path in giga.addons.__path__:
            ...
            if os.path.isfile(tx_path):
                ...
                # first section is [main], after [giga-11.sale]
                tx_sections.extend(tx_config_file.sections()[1:])

            # parent directory ad .tx/config is root directory in giga/giga
        ...
        if not base_url or not tx_sections:
            ...
            # using ini files like '[giga-master.website_sale]'
            ...
            for translation in self:
                ...
                # e.g. https://www.transifex.com/giga/giga-10/translate/#fr/sale/42?q=text:'Sale+Order'
