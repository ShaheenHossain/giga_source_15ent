...
from giga.addons.payment.tests.http_common import PaymentHttpCommon
...
class TestFlows(PaymentCommon, PaymentHttpCommon):
    ...
    def test_transaction_wrong_flow(self):
        ...
        with mute_logger('giga.http'):
            response = self.portal_transaction(**transaction_values)
        self.assertIn(
            "giga.exceptions.UserError: The payment should either be direct, with redirection, or made by a token.",
            response.text)
    ...
    def test_transaction_wrong_token(self):
        ...
        self.assertIn(
            "giga.exceptions.ValidationError: The access token is invalid.",
            response.text)
