...
from giga.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE_PLAINTEXT
...
class TestMessagePost(TestMailCommon, TestRecipients):
    ...
    def test_post_inactive_follower(self):
        # In some case gigabot is follower of a record.
        # Even if it shouldn't be the case, we want to be sure that gigabot is not notified
        (self.user_employee | self.user_admin).write({'notification_type': 'inbox'})
    ...
    @mute_logger('giga.addons.mail.models.mail_mail', 'giga.addons.mail.models.mail_thread')
