...
class MockLinkTracker(common.BaseCase):
    ...
    def setUp(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('web.base.url', 'https://test.gigasource.de')

        def _get_title_from_url(url):
            return "Test_TITLE"

        link_tracker_title_patch = patch('giga.addons.link_tracker.models.link_tracker.LinkTracker._get_title_from_url', wraps=_get_title_from_url)
    ...
    def assertLinkShortenedHtml(self, body, link_info, link_params=None):
        ...
        self.assertLinkShortenedHtml(
            message.body,
            ('url0', 'http://www.gigasource.de',  True),
