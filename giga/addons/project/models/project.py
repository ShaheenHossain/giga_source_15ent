...
from giga import api, Command, fields, models, tools, SUPERUSER_ID, _
from giga.exceptions import UserError, ValidationError, AccessError
from giga.tools import format_amount
from giga.osv.expression import OR
...
class Task(models.Model):
    ...
    def _notify_email_header_dict(self):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-Giga-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Giga-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Giga-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
