...
from giga.exceptions import AccessDenied, AccessError
from giga.http import _request_stack

import giga.tools
from giga.tests import common
from giga.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, giga.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
