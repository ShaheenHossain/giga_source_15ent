...
import giga
from giga import api, http, models, tools, SUPERUSER_ID
from giga.exceptions import AccessDenied, AccessError, MissingError
from giga.http import request, content_disposition, Response
from giga.tools import consteq, pycompat
from giga.tools.mimetypes import guess_mimetype
from giga.modules.module import get_resource_path, get_module_path

from giga.http import ALLOWED_DEBUG_MODES
from giga.tools.misc import str2bool
...
class IrHttp(models.AbstractModel):
    ...
    def _handle_exception(cls, exception):
        ...
        if (isinstance(exception, werkzeug.exceptions.HTTPException) and exception.code == 404) or \
           (isinstance(exception, giga.exceptions.AccessError)):
    ...
    def routing_map(cls, key=None):
        ...
        if key not in cls._routing_map:
            ...
            installed = request.registry._init_modules | set(giga.conf.server_wide_modules)
            if tools.config['test_enable'] and giga.modules.module.current_test:
                installed.add(giga.modules.module.current_test)
