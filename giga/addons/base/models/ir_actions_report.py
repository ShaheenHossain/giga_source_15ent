...
from giga import api, fields, models, tools, SUPERUSER_ID, _
...
from giga.tools import config, is_html_empty
from giga.sql_db import TestCursor
from giga.http import request
from giga.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of Giga so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at Giga start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Giga with at least two workers to print a pdf version of the reports.')
...
class IrActionsReport(models.Model):
    ...
    def _raise_on_unreadable_pdfs(self, streams, stream_record):
        ...
        if unreadable_pdfs:
            ...
            raise UserError(_(
                "Giga is unable to merge the PDFs attached to the following records:\n"
                "%s\n\n"
