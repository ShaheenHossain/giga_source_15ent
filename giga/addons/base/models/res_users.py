...
from giga import api, fields, models, tools, SUPERUSER_ID, _, Command
from giga.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from giga.exceptions import AccessDenied, AccessError, UserError, ValidationError
from giga.http import request
from giga.modules.module import get_module_resource
from giga.osv import expression
from giga.service.db import check_super
from giga.tools import partition, collections, frozendict, lazy_property, image_process
...
class Users(models.Model):
    """ User class. A res.users record models an GigaERP user and is different
        from an employee.
    ...
    def _unlink_except_superuser(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Giga (updates, module installation, ...)'))
        self.clear_caches()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: giga.exceptions.AccessDenied when old password is wrong
        :raise: giga.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
        if not new_passwd:
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your Giga is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Giga in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.gigasource.de/documentation/15.0/administration/install/deploy.html#https for details.",
                    source
                )
