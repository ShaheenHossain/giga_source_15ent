...
from giga.tools import ustr, pycompat, formataddr, email_normalize, encapsulate_email, email_domain_extract, email_domain_normalize
...
class IrMailServer(models.Model):
    ...
    from_filter = fields.Char(
      ...
      'e.g.: "notification@gigasource.de" or "gigasource.de"')
    ...
    def _get_test_email_addresses(self):
        ...
        return email_from, 'noreply@gigasource.de'
    ...
    def connect(self, host=None, port=None, user=None, password=None, encryption=None,
        ...
        """Returns a new SMTP connection to the given SMTP server.
           ...
           :param ssl_certificate: filename of the SSL certificate used for authentication
               Used when no mail server is given and overwrite  the giga-bin argument "smtp_ssl_certificate"
           :param ssl_private_key: filename of the SSL private key used for authentication
               Used when no mail server is given and overwrite  the giga-bin argument "smtp_ssl_private_key"
        ...
        if smtp_encryption == 'ssl':
            if 'SMTP_SSL' not in smtplib.__all__:
                raise UserError(
                    _("Your Giga Source Server does not support SMTP-over-SSL. "
        ...
        # Store the "from_filter" of the mail server / giga-bin argument to  know if we
    ...
    def _get_default_bounce_address(self):
        ...
        If "mail.bounce.alias" is not set it defaults to "postmaster-giga".

        If "mail.catchall.domain" is not set, return None.

        '''
        get_param = self.env['ir.config_parameter'].sudo().get_param
        postmaster = get_param('mail.bounce.alias', default='postmaster-giga')
    ...
    def _find_mail_server(self, email_from, mail_servers=None):
        ...
        - Mail server to use to send the email (None if we use the giga-bin arguments)
        ...
        # 5: SMTP config in giga-bin arguments
        from_filter = tools.config.get('from_filter')
