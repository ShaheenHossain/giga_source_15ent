...
class Meeting(models.Model):
    ...
    @api.model
    def _giga_values(self, google_event, default_reminders=()):
        if google_event.is_cancelled():
            return {'active': False}

        alarm_commands = self._giga_reminders_commands(google_event.reminders.get('overrides') or default_reminders)
        attendee_commands, partner_commands = self._giga_attendee_commands(google_event)
    ...
    @api.model
    def _giga_attendee_commands(self, google_event):
        attendee_commands = []
        partner_commands = []
        ...
        if google_event.exists(self.env):
            existing_attendees = self.browse(google_event.giga_id(self.env)).attendee_ids
        ...
        for giga_attendee in attendees_by_emails.values():
            ...
            email = tools.email_normalize(giga_attendee.email)
            if email not in emails and email != self.env.user.email:
                attendee_commands += [(2, giga_attendee.id)]
                partner_commands += [(3, giga_attendee.partner_id.id)]
        return attendee_commands, partner_commands

    @api.model
    def _giga_reminders_commands(self, reminders=()):
        commands = []
    ...
    def _google_values(self):
        ...
        values = {
            ...
            'extendedProperties': {
                'shared': {
                    '%s_giga_id' % self.env.cr.dbname: self.id,
                },
            },
            'reminders': {
                'overrides': reminders,
        ...
        if self.user_id and self.user_id != self.env.user and not bool(self.user_id.sudo().google_calendar_token):
            # The organizer is an Giga user that do not sync his calendar
            ...
            # the owner is not an Giga user: We don't store the real owner identity (mail)
            ...
            # values['extendedProperties']['private] should be used if the owner is not an giga user
