...
from giga import fields, models, _
from giga.exceptions import UserError
from giga.addons.google_account.models.google_service import GOOGLE_TOKEN_ENDPOINT
from giga.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
from giga.addons.google_calendar.models.google_sync import google_calendar_token
...
class GoogleCredentials(models.Model):
    ...
    calendar_cal_id = fields.Char('Calendar ID', copy=False, help='Last Calendar ID who has been synchronized. If it is changed, we remove all links between GoogleID and Giga Google Internal ID')
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        synced_recurrences = self.env['calendar.recurrence']._sync_google2giga(recurrences)
        synced_events = self.env['calendar.event']._sync_google2giga(events - recurrences, default_reminders=default_reminders)

        # Giga -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_giga2google(calendar_service)
        ...
        (events - synced_events)._sync_giga2google(calendar_service)
