...
from giga.addons.google_calendar.utils.google_event import GoogleEvent
from giga.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from giga.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the Giga database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Giga.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Giga DB (_sync_google2giga).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_giga2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2giga(self, google_events: GoogleEvent, default_reminders=()):
        """Synchronize Google recurrences in Giga. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in Giga
        :return: synchronized giga recurrences
        """
        existing = google_events.exists(self.env)
        new = google_events - existing - google_events.cancelled()

        giga_values = [
            dict(self._giga_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_giga = self.with_context(dont_notify=True)._create_from_google(new, giga_values)
        cancelled = existing.cancelled()
        cancelled_giga = self.browse(cancelled.giga_ids(self.env))
        cancelled_giga._cancel()
        synced_records = new_giga + cancelled_giga
        for gevent in existing - cancelled:
            # Last updated wins.
            # This could be dangerous if google server time and giga server time are different
            updated = parse(gevent.updated)
            giga_record = self.browse(gevent.giga_id(self.env))
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not giga_record.write_date or updated >= pytz.utc.localize(giga_record.write_date):
                vals = dict(self._giga_values(gevent, default_reminders), need_sync=False)
                giga_record.with_context(dont_notify=True)._write_from_google(gevent, vals)
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Giga Source to Google
    ...
    @api.model
    def _giga_values(self, google_event: GoogleEvent, default_reminders=()):
