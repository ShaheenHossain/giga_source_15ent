...
class RecurrenceRule(models.Model):
    ...
    def _write_from_google(self, gevent, vals):
        ...
        # event_tz is written on event in Google but on recurrence in Giga
        vals['event_tz'] = gevent.start.get('timeZone')
        super()._write_from_google(gevent, vals)

        base_event_time_fields = ['start', 'stop', 'allday']
        new_event_values = self.env["calendar.event"]._giga_values(gevent)
        ...
        for giga_attendee_email in existing_attendees.mapped('email'):
            # Remove old attendees
            if email_normalize(giga_attendee_email) not in emails:
                attendee = existing_attendees.filtered(lambda att: att.email == email_normalize(giga_attendee_email))
    ...
    def _create_from_google(self, gevents, vals_list):
        ...
        for gevent, vals in zip(gevents, vals_list):
            base_values = dict(
                self.env['calendar.event']._giga_values(gevent),  # FIXME default reminders
                need_sync=False,
            )
            # If we convert a single event into a recurrency on Google, we should reuse this event on Giga
    ...
    @api.model
    def _giga_values(self, google_recurrence, default_reminders=()):
        return {
