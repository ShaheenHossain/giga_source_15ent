...
from giga.addons.google_calendar.models.res_users import User
from giga.addons.google_calendar.tests.test_sync_common import TestSyncGoogle, patch_api
from giga.tests.common import users, warmup
from giga.tests import tagged

@tagged('giga2google')
@patch.object(User, '_get_google_calendar_token', lambda user: 'dummy-token')
class TestSyncGiga2Google(TestSyncGoogle):
    ...
    def test_event_creation(self):
        ...
        event._sync_giga2google(self.google_service)
        self.assertGoogleEventInserted({
            ...
            'organizer': {'email': 'gigabot@example.com', 'self': True},
    ...
    def test_event_creation_perf(self):
        ...
        with self.assertQueryCount(__system__=1211):
            ...
            events._sync_giga2google(self.google_service)
    ...
    def test_event_allday_creation(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'organizer': {'email': 'gigabot@example.com', 'self': True},
            'attendees': [{'email': 'gigabot@example.com', 'responseStatus': 'accepted'}],
            'extendedProperties': {'shared': {'%s_giga_id' % self.env.cr.dbname: event.id}}
        })
    ...
    def test_recurrence(self):
        ...
        recurrence._sync_giga2google(self.google_service)
        self.assertGoogleEventInserted({
            'id': False,
            'start': {'date': '2020-01-15'},
            'end': {'date': '2020-01-16'},
            ...
            'extendedProperties': {'shared': {'%s_giga_id' % self.env.cr.dbname: recurrence.id}}
        })
    ...
    def test_event_added_to_recurrence(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'attendees': [{'email': 'gigabot@example.com', 'responseStatus': 'accepted'}],
            'recurrence': ['RRULE:FREQ=WEEKLY;COUNT=2;BYDAY=WE'],
            'extendedProperties': {'shared': {'%s_giga_id' % self.env.cr.dbname: event.recurrence_id.id}}
        }, timeout=3)
