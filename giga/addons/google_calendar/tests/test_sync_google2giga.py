...
from giga.addons.google_calendar.tests.test_sync_common import TestSyncGoogle, patch_api
from giga.addons.google_calendar.utils.google_calendar import GoogleEvent
from giga.tools import html2plaintext
from giga import Command

class TestSyncGoogle2Giga(TestSyncGoogle):
    ...
    def sync(self, events):
        ...
        self.env['calendar.recurrence']._sync_google2giga(google_recurrence)
        self.env['calendar.event']._sync_google2giga(events - google_recurrence)
    ...
    def test_new_google_event(self):
        values = {
            ...
            'organizer': {'email': 'gigacalendarref@gmail.com', 'self': True},
        ...
        self.env['calendar.event']._sync_google2giga(GoogleEvent([values]))
    ...
    def test_attendee_cancelled(self):
        ...
        self.env['calendar.event'].with_user(user)._sync_google2giga(gevent)
    ...
    def test_private_extended_properties(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'gigabot@example.com', 'responseStatus': 'declined'}],
            'extendedProperties': {'private': {'%s_giga_id' % self.env.cr.dbname: event.id}},
    ...
    def test_recurrence(self):
        ...
        self.env['calendar.recurrence']._sync_google2giga(GoogleEvent([values]))
    ...
    def test_recurrence_moved(self):
        ...
        values = {
            ...
            "attendees": [
                {
                    "email": "gigabot@example.com", "state": "accepted",
                },
            ],
            'updated': self.now,
        }
    ...
    def test_simple_event_into_recurrency(self):
        ...
        recurrence = self.env['calendar.recurrence']._sync_google2giga(GoogleEvent([values]))
        events = recurrence.calendar_event_ids.sorted('start')
        self.assertEqual(len(events), 3, "it should have created a recurrence with 3 events")
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
    ...
    def test_attendees_same_event_both_share(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'c.c@example.com', 'responseStatus': 'needsAction'},
                          {'email': 'gigabot@example.com', 'responseStatus': 'accepted'},],
            'extendedProperties': {'shared': {'%s_giga_id' % self.env.cr.dbname: event.id,
