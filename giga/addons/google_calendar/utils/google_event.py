...
from giga.tools import email_normalize
import logging
...
class GoogleEvent(abc.Set):
    ...
    Inspired by Giga recordset, one instance can be a single Google event or a
    ...
    def _meta_giga_id(self, dbname):
        """Returns the Giga id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_giga_id' % dbname)
        if o_id:
            return int(o_id)
    ...
    def giga_ids(self, env):
        ...
        found = self._load_giga_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_giga_ids_from_metadata(env, model)

        return tuple(e._giga_id for e in self)

    def _load_giga_ids_from_metadata(self, env, model):
        unsure_giga_ids = tuple(e._meta_giga_id(env.cr.dbname) for e in self)
        giga_events = model.browse(_id for _id in unsure_giga_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Giga id.
        # Therefore, we only consider Giga records without google id when trying
        # to match events.
        o_ids = giga_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            giga_id = e._meta_giga_id(env.cr.dbname)
            if giga_id in o_ids:
                e._events[e.id]['_giga_id'] = giga_id

    def _load_giga_ids_from_db(self, env, model):
        giga_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in giga_events}  # {google_id: giga_id}
        existing_google_ids = giga_events.mapped('google_id')
        for e in self:
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Giga.
        ...
        # (he is the organizer in Google). The "real" owner (in Giga) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Giga several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_giga_ids_from_db(env, env['calendar.recurrence'])
