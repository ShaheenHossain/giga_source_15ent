...
class GigaFinAuth(requests.auth.AuthBase):
    """ This is used to sign the request going towards GigaFin
        e.g.:
            requests.get(EAGLEFIN + '/example', auth=GigaFinAuth())
            By using `auth=GigaFinAuth(self)` when doing a http request, the request will be signed and
    ...
    def __call__(self, request):
        ...
        request.headers.update({
            'gigafin-client-id': self.client_id,
            'gigafin-access-token': self.access_token,
            'gigafin-signature': base64.b64encode(h.digest()),
            'gigafin-timestamp': msg_timestamp,
        })
        return request
