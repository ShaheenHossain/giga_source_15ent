giga.define('point_of_sale.Chrome', function(require) {
    'use strict';
    ...
    const { gigaExceptionTitleMap } = require("@web/core/errors/error_dialogs");
    ...
    class Chrome extends PopupControllerMixin(PosComponent) {
        ...
        _errorHandler(error, errorToHandle) {
            if (errorToHandle instanceof RPCError) {
                ...
                if (gigaExceptionTitleMap.has(errorToHandle.exceptionName)) {
                    const title = gigaExceptionTitleMap.get(errorToHandle.exceptionName).toString();
