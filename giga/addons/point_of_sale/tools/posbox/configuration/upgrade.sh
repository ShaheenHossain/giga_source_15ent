...
copy_raspios () {
    ...
    sed -ie "s/$(echo ${PART_EAGLE_ROOT} | sed -e 's/\//\\\//g')/$(echo ${PART_RASPIOS_ROOT} | sed -e 's/\//\\\//g')/g" raspios/etc/fstab
    ...
    wget -c 'https://nightly.gigasource.de/master/iotbox/iotbox-latest.zip' -O raspios/iotbox-latest.zip
    wget -c 'https://nightly.gigasource.de/master/iotbox/SHA1SUMS.txt' -O raspios/SHA1SUMS.txt
    cd raspios/
...
copy_iot () {
    ...
    # Modify startup
    mkdir -v giga
    mount -v "${PART_IOTBOX_ROOT}" giga
    cp -v /home/pi/upgrade.sh giga/home/pi/
    NBR_LIGNE=$(sed -n -e '$=' giga/etc/rc.local)
    sed -ie "${NBR_LIGNE}"'i\. /home/pi/upgrade.sh; clean_local' giga/etc/rc.local
    find /home/pi/config -maxdepth 1 -type f ! -name ".*" -exec cp {} giga/home/pi/ \;
