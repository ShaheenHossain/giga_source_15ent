...
echo "addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/giga" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -df
cp -a /home/pi/giga/addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/giga/* /home/pi/giga/
rm -r /home/pi/giga/addons/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service giga restart) &
