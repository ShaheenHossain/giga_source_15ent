...
REPO=https://github.com/giga/giga.git
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/giga"
...
giga/
giga-bin" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/giga
echo "${VERSION_IOTBOX}" | tee "${MOUNT_POINT}"/var/giga/iotbox_version "${MOUNT_POINT}"/home/pi/iotbox_version
