...
class Manager(Thread):
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to Giga Source database
        """
        server = helpers.get_giga_server_url()
        if server:
            subject = helpers.read_file_first_line('giga-subject.conf')
            if subject:
            ...
            _logger.warning('Giga server not set')

    def run(self):
        """
        Thread that will load interfaces and drivers and contact the giga server with the updates
        """
