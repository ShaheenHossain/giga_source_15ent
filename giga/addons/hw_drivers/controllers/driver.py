...
from giga import http, tools
from giga.http import send_file
from giga.modules.module import get_resource_path

from giga.addons.hw_drivers.event_manager import event_manager
from giga.addons.hw_drivers.main import iot_devices, manager
...
class DriverController(http.Controller):
    ...
    def connect_box(self, token):
        """
        This route is called when we want that a IoT Box will be connected to a Giga DB
        token is a base 64 encoded string and have 2 argument separate by |
        1 - url of giga DB
        2 - token. This token will be compared to the token of Giga. He have 1 hour lifetime
        """
