...
class IoTRestart(Thread):
    """
    Thread to restart giga server in IoT Box when we must return a answer before
    """
...
def add_credential(db_uuid, enterprise_code):
    write_file('giga-db-uuid.conf', db_uuid)
    write_file('giga-enterprise-code.conf', enterprise_code)

def check_certificate():
    """
    Check if the current certificate is up to date or not authenticated
    """
    server = get_giga_server_url()
    if server:
        ...
        if path.exists():
            with path.open('r') as f:
                ...
                if cn == 'GigaTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected Giga DB and
    ...
    if server:
        ...
        try:
            ...
            if response.status == 200:
                git = ['git', '--work-tree=/home/pi/giga/', '--git-dir=/home/pi/giga/.git']
                ...
                if db_branch != local_branch:
                    ...
                    subprocess.check_call(["rm", "-rf", "/home/pi/giga/addons/hw_drivers/iot_handlers/drivers/*"])
                    subprocess.check_call(["rm", "-rf", "/home/pi/giga/addons/hw_drivers/iot_handlers/interfaces/*"])
...
def check_image():
    ...
    """
    url = 'https://nightly.gigasource.de/master/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def get_giga_server_url():
    ap = subprocess.call(['systemctl', 'is-active', '--quiet', 'hostapd']) # if service is active return 0 else inactive
    if not ap:
        return False
    return read_file_first_line('giga-remote-server.conf')

def get_token():
    return read_file_first_line('token')

def get_version():
    return subprocess.check_output(['cat', '/var/giga/iotbox_version']).decode().rstrip()
...
def load_certificate():
    """
    Send a request to Giga Source with customer db_uuid and enterprise_code to get a true certificate
    """
    db_uuid = read_file_first_line('giga-db-uuid.conf')
    enterprise_code = read_file_first_line('giga-enterprise-code.conf')
    if db_uuid and enterprise_code:
        url = 'https://www.gigasource.de/giga-enterprise/iot/x509'
        data = {
            'params': {
                'db_uuid': db_uuid,
        ...
        if result:
            write_file('giga-subject.conf', result['subject_cn'])
            subprocess.call(["sudo", "mount", "-o", "remount,rw", "/"])
...
def download_iot_handlers(auto=True):
    """
    Get the drivers from the configured Giga server
    """
    server = get_giga_server_url()
    if server:
        urllib3.disable_warnings()
        ...
        try:
            ...
            if resp.data:
                subprocess.call(["sudo", "mount", "-o", "remount,rw", "/"])
                drivers_path = Path.home() / 'giga/addons/hw_drivers/iot_handlers'
...
def load_iot_handlers():
    """
    This method loads local files: 'giga/addons/hw_drivers/iot_handlers/drivers' and
    'giga/addons/hw_drivers/iot_handlers/interfaces'
