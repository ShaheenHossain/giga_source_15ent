giga.define('project_timeshee.ui', function (require ) {
    "use strict";
    ...
    // Desktop detection
    // In Giga, Awesome Timesheet is embedded inside an iframe.
    ...
    var ProjectTimesheet = Widget.extend(ServiceProviderMixin, {
        ...
        */
        ...
        // Giga XML ids require a specific format : module.id.
                                ...
                                .then(function (ids) {
                                    if (ids.length === 0) { // there are no dirty ids on the server
                                        ...
                                        if (!self.data.warning_date || (self.data.warning_date && moment(self.data.warning_date).diff(new Date(), 'days') !== 0)) {
                                            alert('The code on your Giga server is not up to date and an important update has been released. You or your system administrator should consider retrieving it as soon as possible.');
    ...
    var Sync_screen = BasicScreenWidget.extend({
            ...
            _.extend(self.events,
                {
                    ...
                    "click .pt_giga_login_link" : "select_giga_login",
        ...
        select_giga_login: function() {
            var self = this;
            self.server = 'https://www.gigasource.de';
            session.origin = self.server;
            session.setup(self.server, {use_cors : true});
            var always = function() {
                if (session.uid) {
        ...
        show_login_screen: function() {
            this.giga_login_screen = new Giga_login_screen(this);
            this.$('div.pt_sync_screen').empty();
            this.giga_login_screen.appendTo(this.$('div.pt_sync_screen'));
        },
                ...
                if (response.length > 0 && _.contains(response, 'project_timesheet_synchro')) {
                    ...
                    session.rpc('/jsonrpc',  { method : 'server_version' , service : 'db', args : []}).then(function(result) {
                        if (result && result.endsWith('e')) {
                            ...
                            alert("Timesheet Synchronization is available in Giga Enterprise Edition. You should consider upgrading your Giga version if you would like to use it.");
                        } else {
                            alert("The server does not support timesheet synchronization. It requires Giga Enterprise Edition version 9 or newer.");
                        }
                    })
                    self.on_keep_data();
                }
    ...
    var Giga_login_screen = Widget.extend({
        template : "giga_saas_login",
        events: {
            "click .pt_send_giga_login" : "send_giga_login",
        },
        send_giga_login: function() {
            var self = this;
            var login = this.$(".pt_giga_login").val();
            var password = this.$(".pt_giga_password").val();
            var db_name = 'openerp';
            var server_address = 'https://www.gigasource.de';
            ...
            this._rpc({
                ...
                .then(function(res) {
                    var state =  JSON.stringify({
                        ...
                        'p':1, // 1 is the code to use Giga as provider
                    });
    ...
    var Premise_login_url_screen = Widget.extend({
        ...
        validate_url: function() {
            ...
            if (server_address === "gigasource.de" || server_address === "www.gigasource.de") {
                server_address = "www.gigasource.de";
                self.protocol = "https://";
            }
            session.origin = protocol + server_address;
            session.setup(protocol + server_address, {use_cors : true});

            if (server_address === "www.gigasource.de") {
    ...
    var Account_creation_screen = Widget.extend({
        ...
        init: function(parent) {
            ...
            this.giga_is_online = false;
            window.addEventListener("message", this.received_message, false);
            core.bus.on('db_created', this, this.on_db_creation_success);
        },
        /**
         * We perform a quick check (before rendering the widget) to see if Giga is reachable. If not, we don't display an iframe with an ugly 404 message.
        ...
        */
        willStart: function() {
            ...
            return new Promise(function (resolve, reject) {
                $.ajax({
                    url: 'https://www.gigasource.de/fr_FR/trial',
                    type: 'HEAD',
                    cache: false,
                    timeout: 5000,
                    error: function() {
                        self.giga_is_online = false;
                        resolve();
                    },
                    success: function() {
                        self.giga_is_online = true;
                        resolve();
                    },
                });
            });
        ...
        received_message: function(event) {
            if (event.origin === 'https://www.gigasource.de' && event.data === 'success') {
