giga.define('web_editor.snippet.editor', function (require) {
'use strict';
...
var SnippetEditor = Widget.extend({
    ...
    removeSnippet: async function (shouldRecordUndo = true) {
        this.options.wysiwyg.gigaEditor.unbreakableStepUnactive();
        ...
        if (shouldRecordUndo) {
            this.options.wysiwyg.gigaEditor.historyStep();
        }
    },
    /**
    ...
    clone: async function (recordUndo) {
        ...
        if (recordUndo) {
            this.options.wysiwyg.gigaEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.trigger_up('call_for_each_child_snippet', {
                $snippet: $clone,
    ...
    _onDragAndDropStart: function () {
        ...
        this.options.wysiwyg.gigaEditor.automaticStepUnactive();
        var self = this;
        this.dropped = false;
        this._dropSiblings = {
    ...
    _onDragAndDropStop: function (ev, ui) {
        this.options.wysiwyg.gigaEditor.automaticStepActive();
        this.options.wysiwyg.gigaEditor.automaticStepSkipStack();
        this.options.wysiwyg.gigaEditor.unbreakableStepUnactive();
...
var SnippetsMenu = Widget.extend({
    ...
    async start() {
        ...
        $(this.options.wysiwyg.gigaEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.gigaEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.gigaEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            ...
            if ($undoButton.length) {
                const updateHistoryButtons = () => {
                    $undoButton.attr('disabled', !this.options.wysiwyg.gigaEditor.historyCanUndo());
                    $redoButton.attr('disabled', !this.options.wysiwyg.gigaEditor.historyCanRedo());
                };
                this.options.wysiwyg.gigaEditor.addEventListener('historyStep', updateHistoryButtons);
                this.options.wysiwyg.gigaEditor.addEventListener('observerApply', () => {
                    $(this.options.wysiwyg.gigaEditor.editable).trigger('content_changed');
                });
                this.options.wysiwyg.gigaEditor.addEventListener('historyRevert', _.debounce(() => {
    ...
    _makeSnippetDraggable: function ($snippets) {
        ...
        const smoothScrollOptions = this._getScrollOptions({
            jQueryDraggableOptions: {
                ...
                start: function () {
                    self.options.wysiwyg.gigaEditor.automaticStepUnactive();
                ...
                stop: async function (ev, ui) {
                    const doc = self.options.wysiwyg.gigaEditor.document;
                    self.options.wysiwyg.gigaEditor.automaticStepUnactive();
                    self.options.wysiwyg.gigaEditor.automaticStepSkipStack();
                    $toInsert.removeClass('oe_snippet_body');
                    ...
                    if (dropped) {
                        ...
                        _.defer(async function () {
                            ...
                            self.options.wysiwyg.gigaEditor.unbreakableStepUnactive();
                            self.options.wysiwyg.gigaEditor.historyStep();
   ...
   _onSnippetDragAndDropStart: function () {
    ...
    _onSnippetClick() {
        ...
        for (const el of $els) {
            if (!el.children.length) {
                $(el).gigaBounce('o_we_snippet_area_animation');
            }
        }
    },
    /**
    ...
    _addToolbar(toolbarMode = "text") {
        ...
        this.options.wysiwyg.gigaEditor.bindExecCommand($customizeTableBlock[0]);
    ...
    _checkEditorToolbarVisibility: function (e) {
        ...
        const selection = this.options.wysiwyg.gigaEditor.document.getSelection();
    ...
    _onRequestEditable: function (ev) {
        ev.data.callback($(this.options.wysiwyg.gigaEditor.editable));
    },
    /**
