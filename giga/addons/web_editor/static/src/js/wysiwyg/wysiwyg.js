giga.define('web_editor.wysiwyg', function (require) {
'use strict';
...
const GigaEditorLib = require('@web_editor/../lib/giga-editor/src/GigaEditor');
...
const GigaEditor = GigaEditorLib.GigaEditor;
const getDeepRange = GigaEditorLib.getDeepRange;
const getInSelection = GigaEditorLib.getInSelection;
const isBlock = GigaEditorLib.isBlock;
const rgbToHex = GigaEditorLib.rgbToHex;
const preserveCursor = GigaEditorLib.preserveCursor;
const closestElement = GigaEditorLib.closestElement;
...
const Wysiwyg = Widget.extend({
    ...
    start: async function () {
        ...
        this.gigaEditor = new GigaEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        if ($wrapwrap.length) {
            $wrapwrap[0].addEventListener('scroll', this.gigaEditor.multiselectionRefresh, { passive: true });
        }
        ...
        this._observeGigaFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.gigaEditor.observerUnactive();
                const $field = $(this);
                ...
                self.gigaEditor.observerActive();
            }
        );
        ...
        if (options.snippets) {
            $(this.gigaEditor.document.body).addClass('editor_enable');
        ...
        $(this.gigaEditor.editable).on('click', this._updateEditorUI.bind(this));
        $(this.gigaEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.gigaEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        // Ensure the Toolbar always have the correct layout in note.
        this._updateEditorUI();

        return _super.apply(this, arguments).then(() => {
            if (this.options.autohideToolbar) {
                if (this.gigaEditor.isMobile) {
                    $(this.gigaEditor.editable).before(this.toolbar.$el);
                } else {
    ...
    setupCollaboration(collaborationChannel) {
        ...
        this._peerToPeerLoading = new Promise(async (resolve) => {
            ...
            this.ptp = new PeerToPeer({
                ...
                onRequest: {
                    ...
                    get_missing_steps: (params) => this.gigaEditor.historyGetMissingSteps(params.requestPayload),
                    get_history_from_snapshot: () => this.gigaEditor.historyGetSnapshotSteps(),
                    get_collaborative_selection: () => this.gigaEditor.getCurrentCollaborativeSelection(),
                },
                onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                    switch (notificationName) {
                        case 'ptp_remove':
                            this.gigaEditor.multiselectionRemove(notificationPayload);
                            break;
                        case 'ptp_disconnect':
                            this.ptp.removeClient(fromClientId);
                            this.gigaEditor.multiselectionRemove(fromClientId);
                            break;
                        case 'rtc_data_channel_open':
                            ...
                            if (!historySyncAtLeastOnce) {
                                ...
                                if (remoteSelection) {
                                    this.gigaEditor.onExternalMultiselectionUpdate(remoteSelection);
                                }
                            }
                            break;
                        case 'oe_history_step':
                            ...
                            if (historySyncFinished) {
                                this.gigaEditor.onExternalHistorySteps([notificationPayload]);
                            }
                            break;
                        case 'oe_history_set_selection':
                            ...
                            this.gigaEditor.onExternalMultiselectionUpdate(selection);
                            break;
                    }
                }
            });
        ...
        const editorCollaborationOptions = {
            ...
            onHistoryMissingParentSteps: async ({ step, fromStepId }) => {
                ...
                this.ptp && this.gigaEditor.onExternalHistorySteps(missingSteps.concat([step]));
            },
            onHistoryNeedSync: async () => {
                ...
                if (firstClientId !== currentClientId) {
                    ...
                    this.gigaEditor.historyResetFromSteps(historySteps);
    ...
    destroy: function () {
        ...
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $('#wrapwrap')[0].removeEventListener('scroll', this.gigaEditor.multiselectionRefresh, { passive: true });
        }
        this._super();
    },
    /**
    ...
    renderElement: function () {
        ...
        if (this.options.resizable && !device.isMobile) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper giga-editor">');
    ...
    getValue: function (options) {
        ...
        this.gigaEditor.cleanForSave($editable[0]);
    ...
    cancel: function (reload) {
        ...
        return new Promise((resolve, reject) => {
            if (!this.gigaEditor.historySize().length) {
                resolve();
            } else {
    ...
    setValue: function (value) {
        ...
        this.gigaEditor.sanitize();
    },
    /**
     * Undo one step of change in the editor.
     */
    undo: function () {
        this.gigaEditor.historyUndo();
    },
    /**
     * Redo one step of change in the editor.
     */
    redo: function () {
        this.gigaEditor.historyRedo();
    },
    /**
     ...
     */
    focus: function () {
        if (!this.gigaEditor.historyResetLatestComputedSelection()) {
            ...
            const selection = this.gigaEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    },
    /**
     * Start or resume the Giga field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeGigaFieldChanges: function () {
        ...
        if (this.gigaFieldObservers) {
            for (let observerData of this.gigaFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const gigaFieldSelector = '[data-oe-model], [data-oe-translation-id]';
            const $gigaFields = this.$editable.find(gigaFieldSelector);
            this.gigaFieldObservers = [];

            $gigaFields.each((i, field) => {
                const observer = new MutationObserver(() => {
                    let $node = $(field);
                    let $nodes = $gigaFields.filter(function () {
                        return this !== field;
                    });
                ...
                this.gigaFieldObservers.push({field: field, observer: observer});
            });
        }
    },
    /**
     ...
     */
    _pauseGigaFieldObservers: function () {
        for (let observerData of this.gigaFieldObservers) {
    ...
    toggleLinkTools(options = {}) {
        if (this.snippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.linkTools) {
                ...
                this.linkTools = new weWidgets.LinkTools(this, {wysiwyg: this, noFocusUrl: options.noFocusUrl}, this.gigaEditor.editable, {}, $btn, options.link || this.lastMediaClicked);
                const _onMousedown = ev => {
                    if (!ev.target.closest('.oe-toolbar') && !ev.target.closest('.ui-autocomplete')) {
                        ...
                        this.gigaEditor.document.removeEventListener('mousedown', _onMousedown, true);
                    }
                };
                this.gigaEditor.document.addEventListener('mousedown', _onMousedown, true);
            ...
            const restoreSelection = preserveCursor(this.gigaEditor.document);
            linkDialog.open();
            linkDialog.on('save', this, data => {
                ...
                // set the focus.
                setTimeout(() => this.gigaEditor.document.getSelection().collapseToEnd(), 0);
            });
    ...
    openMediaDialog(params = {}) {
        const sel = this.gigaEditor.document.getSelection();
        const fontawesomeIcon = getInSelection(this.gigaEditor.document, '.fa');
        ...
        const $editable = $(GigaEditorLib.closestElement(range.startContainer, '.o_editable'));
        ...
        mediaDialog.on('save', this, function (element) {
            ...
            if (wysiwygUtils.isImg($node[0])) {
                ...
                this.gigaEditor.unbreakableStepUnactive();
                this.gigaEditor.historyStep();
            } else if (element) {
                this.gigaEditor.execCommand('insertHTML', element.outerHTML);
            }
        });
    ...
    _configureToolbar: function (options) {
        ...
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.gigaEditor.document});
            const mouseup = () => {
                $('#image-transform').toggleClass('active', $image.is('[style*="transform"]'));
            };
            $(this.gigaEditor.document).on('mouseup', mouseup);
            const mousedown = mousedownEvent => {
                if (!$(mousedownEvent.target).closest('.transfo-container').length) {
                    $image.transfo('destroy');
                    $(this.gigaEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                }
            ...
            $(this.gigaEditor.document).on('mousedown', mousedown);
        });
        ...
        setTimeout(() => {
            ...
            if (!options.snippets && scrollableContainer.length) {
                // this.gigaEditor.addDomListener(
                //   scrollableContainer[0],
                //   'scroll',
                //   this.gigaEditor.updateToolbarPosition.bind(this.gigaEditor),
                // );
            }
        }, 0);
    },
            ...
            $dropdown.on('show.bs.dropdown', () => {
                ...
                mutex.exec(() => {
                    ...
                    colorpicker = new ColorPaletteWidget(this, {
                        ...
                        $editable: $(this.gigaEditor.editable), // Our parent is the root widget, we can't retrieve the editable section from it...
                    ...
                    colorpicker.on('custom_color_picked color_picked', null, ev => {
                        if (hadNonCollapsedSelection) {
                            this.gigaEditor.historyResetLatestComputedSelection(true);
                        }
                    ...
                    colorpicker.on('color_hover color_leave', null, ev => {
                        ...
                        this.gigaEditor.historyPauseSteps();
                        try {
                            this._processAndApplyColor(eventName, ev.data.color);
                        } finally {
                            this.gigaEditor.historyUnpauseSteps();
                        }
                    });
    ...
    _processAndApplyColor: function (eventName, color) {
        ...
        this.gigaEditor.execCommand('applyColor', color, eventName === 'foreColor' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        const hexColor = this._colorToHex(color);
        this.gigaEditor.updateColorpickerLabels({
    ...
    _updateEditorUI: function (e) {
        ...
        if (isInMedia) {
            ...
            const selection = this.gigaEditor.document.getSelection();
            const range = this.gigaEditor.document.createRange();
        ...
        const link = getInSelection(this.gigaEditor.document, 'a');
        if (isInMedia || link) {
    ...
    _updateMediaJustifyButton: function (commandState) {
        ...
        // Change the ID to prevent GigaEditor from controlling it as this is
        ...
        const commands = [
            {
                ...
                callback: () => {
                    this.gigaEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                groupName: 'Basic blocks',
                ...
                fontawesome: 'fa-code',
                callback: () => {
                    this.gigaEditor.execCommand('setTag', 'pre');
                },
            },
            {
    ...
    _getSnippetsCommands: function () {
        const snippetCommandCallback = (selector) => {
            ...
            const range = getDeepRange(this.gigaEditor.editable);
...
giga.define('web_editor.widget', function (require) {
'use strict';
    return {
