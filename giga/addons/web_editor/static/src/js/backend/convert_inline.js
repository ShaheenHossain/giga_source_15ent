giga.define('web_editor.convertInline', function (require) {
'use strict';
...
// Note: duplicated from giga-editor utils.
...
function fontToImg($editable) {
    var fonts = giga.__DEBUG__.services["wysiwyg.fonts"];
...
FieldHtml.include({
    ...
    _toInline: function () {
        ...
        const $gigaEditor = $editable.closest('.giga-editor');
        // Remove temporarily the class so that css editing will not be converted.
        $gigaEditor.removeClass('giga-editor');
        $editable.html(html);
        ...
        $gigaEditor.addClass('giga-editor');
