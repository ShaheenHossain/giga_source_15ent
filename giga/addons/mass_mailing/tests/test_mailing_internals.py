...
from giga.addons.base.tests.test_ir_cron import CronMixinCase
from giga.addons.mass_mailing.tests.common import MassMailCommon
from giga.tests.common import users, Form
from giga.tools import formataddr, mute_logger
...
class TestMassMailFeatures(MassMailCommon, CronMixinCase):
    ...
    def test_mailing_shortener(self):
        mailing = self.env['mailing.mailing'].create({
            ...
            'body_html': """<div>
Hi,
<t t-set="url" t-value="'www.gigasource.de'"/>
<t t-set="httpurl" t-value="'https://www.giga.eu'"/>
Website0: <a id="url0" t-attf-href="https://www.giga.tz/my/{{object.name}}">https://www.giga.tz/my/<t t-esc="object.name"/></a>
Website1: <a id="url1" href="https://www.giga.be">https://www.giga.be</a>
...
Email: <a id="url5" href="mailto:test@gigasource.de">test@gigasource.de</a></div>""",
            'mailing_model_id': self.env['ir.model']._get('mailing.list').id,
            'reply_to_mode': 'new',
            'reply_to': self.email_reply_to,
            'contact_list_ids': [(6, 0, self.mailing_list_1.ids)],
        ...
        for contact in self.mailing_list_1.contact_ids:
            ...
            for link_info in [('url0', 'https://www.giga.tz/my/%s' % contact.name, True),
                              ('url1', 'https://www.giga.be', True),
                              ('url2', 'https://www.gigasource.de', True),
                              ('url3', 'https://www.giga.eu', True),
                              ('url4', 'https://www.example.com/foo/bar?baz=qux', True),
                              ('url5', 'mailto:test@gigasource.de', False)]:
