    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            if '_module' not in attrs:
                ...
                assert module.startswith('giga.addons.'), \
                    f"Invalid import of {module}.{name}, it should start with 'giga.addons'."
...
class BaseModel(metaclass=MetaModel):
    """Base class for Giga models.

    Giga models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~giga.models.BaseModel._register` attribute may be set to False.
    """
    __slots__ = ['env', '_ids', '_prefetch_ids']

    _auto = False
    """Whether a database table should be created.
    If set to ``False``, override :meth:`~giga.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~giga.models.AbstractModel`.
    """
    ...
    _module = None              #: the model's module (in the Giga sense)
    ...
    .. warning::
      ...
      :attr:`~giga.models.Model._inherits`-ed models, the inherited field will
    ...
    def _compute_concurrency_field(self):
        ...
        if self._log_access:
            ...
            self[fname] = giga.fields.Datetime.now()
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], giga.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], giga.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from giga.fields import Char, Integer
    ...
    def _read_group_fill_temporal(self, data, groupby, aggregated_fields, annotated_groupbys,
        ...
        if fill_from:
            fill_from = date_utils.start_of(giga.fields.Datetime.to_datetime(fill_from), granularity)
            if tz:
                fill_from = tz.localize(fill_from)
        elif existing_from:
            fill_from = existing_from
        if fill_to:
            fill_to = date_utils.start_of(giga.fields.Datetime.to_datetime(fill_to), granularity)
            if tz:
...
%(fields_list)s""") % {
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~giga.addons.base.models.res_company`).

        For :class:`~giga.addons.base.models.res_users` relational fields,
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~giga.fields.Integer`,
          :class:`~giga.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~giga.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~giga.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~giga.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~giga.fields.Date` and
              :class:`~giga.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`giga.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`giga.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _openerp/models/relationals/format:

          The expected value of a :class:`~giga.fields.One2many` or
          :class:`~giga.fields.Many2many` relational field is a list of
          :class:`~giga.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~giga.fields.Command.create`,
          :meth:`~giga.fields.Command.update`,
          :meth:`~giga.fields.Command.delete`,
          :meth:`~giga.fields.Command.unlink`,
          :meth:`~giga.fields.Command.link`,
          :meth:`~giga.fields.Command.clear`, and
          :meth:`~giga.fields.Command.set`.
          """
        if not self:
            return True
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an GigaERP domain.
    ...
    def ensure_one(self):
        ...
        :raise giga.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~giga.api.Environment`
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~giga.addons.base.models.res_company` or int
        ...
        for field, value in sorted(field_values, key=lambda item: item[0].write_sequence):
            ...
            if field.relational:
                ...
                for invf in self.pool.field_inverses[field]:
                    ...
                    # /home/dle/src/giga/master-nochange-fp/giga/addons/test_new_api/tests/test_new_fields.py
    ...
    def _populate_factories(self):
        ...
        See :mod:`giga.tools.populate` for population tools and applications.
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted Giga models.

    Giga models are created by inheriting from this class::
