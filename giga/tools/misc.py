...
"""
Miscellaneous tools used by GigaERP.
"""
import cProfile
import collections
import datetime
...
from giga.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if giga.tools.config['db_host']:
        env['PGHOST'] = giga.tools.config['db_host']
    if giga.tools.config['db_port']:
        env['PGPORT'] = str(giga.tools.config['db_port'])
    if giga.tools.config['db_user']:
        env['PGUSER'] = giga.tools.config['db_user']
    if giga.tools.config['db_password']:
        env['PGPASSWORD'] = giga.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',)):
    ...
    addons_paths = giga.addons.__path__ + [root_path]
...
def file_open(name, mode="r", filter_ext=None):
    ...
    >>> with file_open('/opt/giga/addons/hr/static/description/icon.png', 'rb') as f:
    ...     contents = f.read()
...
def scan_languages():
    """ Returns all languages supported by GigaERP for translation

    :returns: a list of (lang_code, lang_name) pairs
    :rtype: [(str, unicode)]
    """
    csvpath = giga.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns
       ...
       the evaluation.
       Mostly useful for evaluating GigaERP domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('giga.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('giga.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = giga.fields.Datetime.from_string(value)
            value = giga.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = giga.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = giga.fields.Datetime.from_string(value)
    else:
        timestamp = value
