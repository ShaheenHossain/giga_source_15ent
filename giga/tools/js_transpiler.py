"""
This code is what let us use ES6-style modules in giga.
Classic Giga modules are composed of a top-level :samp:`giga.define({name},{body_function})` call.
This processor will take files starting with an `@giga-module` annotation (in a comment) and convert them to classic modules.
If any file has the /** giga-module */ on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual giga one with minimal changes.
...
def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom giga modules.
    ...
    legacy_giga_define = get_aliased_giga_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_giga_define, module_path),
    ]
    for s in steps:
        content = s(content)
    if legacy_giga_define:
        content += legacy_giga_define
    return content
...
def url_to_module_path(url):
    """
    Giga modules each have a name. (giga.define("<the name>", async function (require) {...});
...
def wrap_with_giga_define(module_path, content):
    """
    Wraps the current content (source code) with the giga.define call.
    Should logically be called once all other operations have been performed.
    """
    return f"""giga.define({module_path!r}, async function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
EAGLE_MODULE_RE = re.compile(r"""
    \s*                                       # some starting space
    \/(\*|\/).*\s*                            # // or /*
    @giga-module                              # @giga-module
...
def is_giga_module(content):
    """
    Detect if the file is a native giga module.
    We look for a comment containing @giga-module.

    :param content: source code
    :return: is this a giga module that need transpilation ?
    """
    result = EAGLE_MODULE_RE.match(content)
    return bool(result)


def get_aliased_giga_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the giga source base being:
    ...
    > giga.define("web/chrome/abstract_action", async function(require) {
    ...
    .. code-block:: javascript
        ...
        /** @giga-module */
        // after
        /** @giga-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @giga-module */
        // after
        /** @giga-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = EAGLE_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\ngiga.define(`%s`, async function(require) {
                        return require('%s');
                        });\n""" % (alias, module_path)
            else:
                return """\ngiga.define(`%s`, async function(require) {
