...
GigaERP domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    an GigaERP domain expression from an untrusted source.
    ...
    except giga.exceptions.UserError:
        raise
    except giga.exceptions.RedirectWarning:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except giga.http.AuthenticationError:
        raise
...
`giga.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `giga.tools.safe_eval`.
""")
    return d
