...
# *broken*
NO_POSTMORTEM = (giga.exceptions.AccessDenied,
                 giga.exceptions.UserError,
                 giga.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                giga.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = giga.service.common.dispatch
        elif service_name == 'db':
            dispatch = giga.service.db.dispatch
        elif service_name == 'object':
            dispatch = giga.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                giga.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                giga.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except Exception as e:
        _logger.exception(giga.tools.exception_to_unicode(e))
        giga.tools.debugger.post_mortem(giga.tools.config, sys.exc_info())
        raise


class WebRequest(object):
    """ Parent class for all Giga Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in giga.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~giga.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~giga.api.Environment` bound to current request. """
        if self._env is None:
            self._env = giga.api.Environment(self.cr, self.uid, self.context)
        return self._env

    @lazy_property
    def session(self):
        """ :class:`GigaERPSession` holding the HTTP session data for the
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            giga.tools.debugger.post_mortem(
                giga.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        Giga implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <giga.http.WebRequest.csrf_token`, the
          :data:`~giga.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          Giga) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (giga.exceptions.UserError,
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "Giga Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "Giga Session Expired"
...
Giga URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.gigasource.de/documentation/15.0/developer/reference/addons/http.html#csrf for
more details.

* if this endpoint is accessed through Giga via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["giga", "addons"]:
            module = ""
        else:
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('giga.addons.') and c.__module__.split(".")[2] in modules
...
class GigaERPSession(sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(GigaERPSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def authenticate(self, db, login=None, password=None):
        ...
        uid = giga.registry(db)['res.users'].authenticate(db, login, password, env)
        self.pre_uid = uid
    ...
    def check_security(self):
        ...
        env = giga.api.Environment(request.cr, self.uid, self.context)
    ...
    def _fix_lang(self, context):
        """ GigaERP provides languages which may not make sense and/or may not
...
EAGLE_DISABLE_SESSION_GC = str2bool(os.environ.get('EAGLE_DISABLE_SESSION_GC', '0'))

if EAGLE_DISABLE_SESSION_GC:
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or giga.SUPERUSER_ID)
...
class Root(object):
    """Root WSGI application for the GigaERP Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def load_addons(self):
        ...
        for addons_path in giga.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request_manager:
                ...
                if db:
                    try:
                        giga.registry(db).check_signaling()
                        with giga.tools.mute_logger('giga.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = giga.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if giga.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = giga.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif giga.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Giga will
        # use the value of --database as a comma separated list of exposed databases.
        exposed_dbs = set(db.strip() for db in giga.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = giga.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = giga.tools.ustr(filename)
