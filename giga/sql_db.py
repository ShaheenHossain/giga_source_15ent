...
"""
The PostgreSQL connector is a connectivity layer between the GigaERP code and
...
def flush_env(cr, *, clear=True):
    warnings.warn("Since Giga 15.0, use cr.flush() instead of flush_env(cr).",
...
def clear_env(cr):
    warnings.warn("Since Giga 15.0, use cr.clear() instead of clear_env(cr).",
                  DeprecationWarning, stacklevel=2)
    cr.clear()
...
class Cursor(BaseCursor):
    """Represents an open transaction to the PostgreSQL DB backend,
       ...
       ``cursor`` objects.
        ...
        such as GigaERP, we need the best guarantees that no data
        ...
        GigaERP implements its own level of locking protection
        ...
        the default transaction isolation level for GigaERP cursors, as
    ...
    def autocommit(self, on):
        if on:
            warnings.warn(
                "Since Giga 13.0, the ORM delays UPDATE queries for "
...
def close_db(db_name):
    """ You might want to call giga.modules.registry.Registry.delete(db_name) along this function."""
    global _Pool
    if _Pool:
