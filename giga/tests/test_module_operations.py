...
import giga
from giga.tools import topological_sort, unique
from giga.netsvc import init_logger
from giga.tests import standalone_tests
import giga.tests.loader

_logger = logging.getLogger('giga.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with giga.registry(db_name).cursor() as cr:
        env = giga.api.Environment(cr, giga.SUPERUSER_ID, {})
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of Giga modules")
    ...
    parser.add_argument("--addons-path", "-p", type=str,
        help="Comma-separated list of paths to directories containing extra Giga modules")
...
def test_full(args):
    ...
    with giga.registry(args.database).cursor() as cr:
        env = giga.api.Environment(cr, giga.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = giga.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        giga.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        giga.tools.config['addons_path'] = ','.join([args.addons_path, giga.tools.config['addons_path']])
        if args.data_dir:
            giga.tools.config['data_dir'] = args.data_dir
        giga.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('giga.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('giga.sql_db').setLevel(logging.CRITICAL)
