...
from .. import tools
from .common import TagsSelector, GigaSuite
from .runner import GigaTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules('giga.addons', module)

    try:
        importlib.import_module('giga.upgrade.%s' % module)
    except ImportError:
        pass
    else:
        results += _get_tests_modules('giga.upgrade', module)
...
def make_suite(module_names, position='at_install'):
    ...
    return GigaSuite(sorted(tests, key=lambda t: t.test_sequence))
