# -*- coding: utf-8 -*-
"""
The module :mod:`giga.tests.common` provides unittest test cases and a few
...
from giga.models import BaseModel
from giga.exceptions import AccessError
from giga.modules.registry import Registry
from giga.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from giga.service import security
from giga.sql_db import BaseCursor, Cursor
from giga.tools import float_compare, single_email_re, profiler
from giga.tools.misc import find_in_path
from giga.tools.safe_eval import safe_eval
...
# The giga library is supposed already configured.
ADDONS_PATH = giga.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = giga.SUPERUSER_ID


def get_db_name():
    db = giga.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding Giga module name.
    """
    def register(func):
        # register func by giga module name
        if func.__module__.startswith('giga.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most giga operations;
    """
    if not login:
...
# Main classes
# ------------------------------------------------------------
class GigaSuite(unittest.suite.TestSuite):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        from giga.http import root
        if not root._loaded:
            root.load_addons()
            root._loaded = True
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('giga.addons.'):
...
class BaseCase(unittest.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for Giga-specific code. This class is abstract and
    ...
    longMessage = True      # more verbose error message by default: https://www.gigasource.de/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~giga.models.BaseModel`
        """
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('giga.sql_db.Cursor.execute', execute):
            with patch('giga.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/giga/addons/" in filename:
                        filename = filename.rsplit("/giga/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.registry = giga.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, giga.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_giga')
        self.chrome_pid = None

        otc = giga.tools.config
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way Giga widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.xmlrpc_url = f'http://{HOST}:{giga.tools.config["http_port"]:d}/xmlrpc/2/'
    ...
    def url_open(self, url, data=None, files=None, timeout=10, headers=None, allow_redirects=True, head=False):
        if url.startswith('/'):
            url = "http://%s:%s%s" % (HOST, giga.tools.config['http_port'], url)
        if head:
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('giga.service.http.request.')]
        ...
        if request_threads:
            ...
            giga.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=True)
        giga.http.root.session_store.save(self.session)

    def authenticate(self, user, password):
        db = get_db_name()
        if getattr(self, 'session', None):
            giga.http.root.session_store.delete(self.session)

        self.session = session  = giga.http.root.session_store.new()
        session.db = db
        ...
        giga.http.root.session_store.save(session)
    ...
    @classmethod
    def base_url(cls):
        return "http://%s:%s" % (HOST, giga.tools.config['http_port'])
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "giga.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "giga.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~giga.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~giga.fields.Many2many` fields, the field itself is a
    :class:`~giga.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~giga.fields.One2many` are reified as
    :class:`~giga.tests.common.O2MProxy`.

    Because the :class:`~giga.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~giga.tests.common.O2MProxy.new` and
    :meth:`~giga.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: giga.models.Model
    ...
    :type view: int | str | giga.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~giga.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~giga.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = giga.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = giga.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return giga.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return giga.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from giga.tests.common have a test_tags
