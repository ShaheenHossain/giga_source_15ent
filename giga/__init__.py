...
""" GigaERP core library."""


#----------------------------------------------------------
# giga must be a namespace package for giga.addons to become one too
...
import sys
assert sys.version_info > (3, 6), "Outdated python version detected, Giga requires Python >= 3.6 to run."
...
if len(sys.argv) > 1 and sys.argv[1] == 'gevent':
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in giga/debian/copyright)
        while 1:
            state = conn.poll()
...
# libc UTC hack
#----------------------------------------------------------
# Make sure the GigaERP server runs in UTC.
import os
...
from . import fields
from . import api
from giga.tools.translate import _, _lt
