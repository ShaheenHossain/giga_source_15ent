...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('giga.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='giga')
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='giga.tools.image')
    ...
    # Check that handler.stream has a fileno() method: when running GigaERP
...
DEFAULT_LOG_CONFIGURATION = [
    'giga.http.rpc.request:INFO',
    'giga.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['giga:DEBUG', 'giga.sql_db:INFO', 'giga.http.rpc:DEBUG'],
    'debug_rpc': ['giga:DEBUG', 'giga.sql_db:INFO', 'giga.http.rpc.request:DEBUG'],
    'debug': ['giga:DEBUG', 'giga.sql_db:INFO'],
    'debug_sql': ['giga.sql_db:DEBUG'],
    'info': [],
    'runbot': ['giga:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['giga:WARNING', 'werkzeug:WARNING'],
    'error': ['giga:ERROR', 'werkzeug:ERROR'],
    'critical': ['giga:CRITICAL', 'werkzeug:CRITICAL'],
}
